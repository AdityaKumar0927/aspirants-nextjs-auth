export async function getOrder(id: string) {
  return (await getissues()).find((order) => order.id.toString() === id)!
}

export async function getRecentissues() {
  return (await getissues()).slice(0, 10)
}
...
export async function getEventissues(id: string) {
  return (await getissues()).filter((order) => order.event.id.toString() === id)
}
