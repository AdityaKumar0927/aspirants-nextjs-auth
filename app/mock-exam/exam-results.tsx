// File: /app/mock-exam/exam-results.tsx
"use client"
...
export default function AdvancedExamResults({
  ...
  // Sort topicPerformance by topic name
  ...
  // Sort topicWiseIncorrectAnswers
  ...
  function generatePdfContent(){
    ...
    container.innerHTML = `
      <h1>Exam Results</h1>
      <p>Total Q: ${examResults.totalQuestions}</p>
  ...
  const StrengthsAndWeaknesses = () => (
    <div className="grid gap-6 md:grid-cols-2">
      ...
      <Card className="bg-gradient-to-br from-green-100 to-green-50 dark:from-green-900/20 dark:to-green-800/20">
        ...
        <CardContent className="p-6">
          <ul className="space-y-4">
            {examResults.topStrengths.map(([topic, performance], i)=>(
      ...
      <Card className="bg-gradient-to-br from-yellow-100 to-yellow-50 dark:from-yellow-900/20 dark:to-yellow-800/20">
        ...
        <CardContent className="p-6">
          <ul className="space-y-4">
            {examResults.topWeaknesses.map(([topic, performance], i)=>(
              <li key={topic} className="flex items-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <ScrollArea className="h-[300px] pr-4">
            <div className="space-y-6">
              {sortedTopicPerformance.map(([topic, p])=>(
                <div key={topic}>
                  <div className="flex justify-between text-sm mb-2">
                    <span className="font-medium">{topic}</span>
                    <span className="font-bold">
                      {((p.correct/p.total)*100).toFixed(2)}%
                    </span>
                  </div>
                  <Progress value={(p.correct/p.total)*100} className="h-2"/>
                </div>
              ))}
            </div>
          </ScrollArea>
        ...
        <Bar
          ...
          options={{
            ...
            plugins:{
              ...
              title:{ display:true, text:"Incorrect Answers by Topic (A→Z)" },
            }
          }}
        />
      </CardContent>
  ...
  // 5) "Answer Review" => an Accordion for each question, with "question bank style"
  const AnswerReview = () => (
    <Card>
      <CardHeader className="border-b">
        <CardTitle className="text-2xl font-semibold tracking-tight flex items-center">
        ...
        <CardDescription>
          ...
          <div className="flex space-x-2 mt-2">
            {["All","Easy","Medium","Hard"].map((df)=>(
              <Badge
                key={df}
                variant={difficultyFilter===df?"default":"outline"}
        ...
        <ScrollArea className="h-[400px]">
          <Accordion type="single" collapsible className="w-full space-y-2">
            {filteredQuestions.map((q, i)=>{
              const userAns = examResults.userAnswers[i] || "";
              const correctAns = q.correctOption || "";
              const isCorrect = userAns === correctAns;
              const timeSpent = examResults.timeSpentPerQuestion[i] || 0;
                ...
                >
                  ...
                  <AccordionContent>
                    <div className="mt-2 space-y-3">
                      {/* question text */}
                      {q.text && (
                        <div className="text-base leading-6 latex-font">
                          <MathRenderer text={q.text}/>
                        </div>
                      )}
                      {/* If MCQ => highlight user selection */}
                      {Array.isArray(q.options) && q.options.length>0 && (
                        <div className="space-y-2">
                          {q.options.map((optText, idxOpt)=>{
                            const letter = String.fromCharCode(65+ idxOpt);
                            const isUser = userAns=== letter;
                            const isCorrectLetter = letter === correctAns;
                            let styleClasses = "border-gray-300";
                            if(isUser && isCorrect){
                              styleClasses = "bg-green-100 border-green-300 text-green-800";
                            } else if(isUser && !isCorrect){
                              styleClasses = "bg-red-100 border-red-300 text-red-800";
                            }
                            return (
                              <div
                                key={idxOpt}
                                className={`border p-2 rounded ${styleClasses}`}
                              >
                      ...
                      {/* If numeric => just show userAns */}
                      {(q.type||"").toLowerCase().includes("num") && (
                        <p>
                          Your numeric answer:
                          <span className={isCorrect?"text-green-700":"text-red-700"}> {userAns}</span>
                        </p>
                      )}
                      {/* If user is wrong => show correct explicitly */}
                      ...
                      {/* explanation */}
                      {q.explanation && (
                        <div className="mt-2 p-2 rounded bg-gray-50 text-sm text-gray-700">
  ...
  return (
    ...
    >
      ...
      {/* Sticky top => PDF button */}
      <div className="sticky top-0 bg-background z-10 p-4 mb-8 border-b flex flex-col gap-3 sm:flex-row sm:justify-between sm:items-center">
      ...
      <Tabs defaultValue="summary" className="w-full">
        ...
        >
          ...
          {/* 3) review => new accordion-based UI */}
      ...
      {/* Bottom CTA => exit / start new exam */}
      <motion.div
