...
export default function FeedbackPopover({ questionId }: FeedbackPopoverProps) {
  ...
  return (
    <Popover>
      ...
      <PopoverContent className={`w-full max-w-[480px] p-0 ${isDarkTheme ? 'bg-zinc-900 text-white' : 'bg-white text-black'}`}>
        <div className="p-4 space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h2 className={`text-xl font-semibold ${isDarkTheme ? 'text-white' : 'text-black'}`}>Question Feedback</h2>
              <p className={`text-sm ${isDarkTheme ? 'text-zinc-400' : 'text-gray-500'}`}>
            ...
            >
              {["easy", "medium", "hard"].map((difficulty) => (
                <div key={difficulty} className="flex items-center space-x-2">
                  ...
                  />
                  <Label htmlFor={difficulty} className="capitalize text-sm">{difficulty}</Label>
                </div>
              ))}
            </RadioGroup>
          </div>
